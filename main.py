                ...
                def monitor():
                    ...
                    with open(log_file, "a") as f:
                        f.write(
                            ...
                            f"Process ended with code {exit_code}\n"
                        )
...
def generate_file_manager_link(user_id, project_id):
    ...
    now = datetime.now()
    expired = [
        k for k, v in file_manager_sessions.items()
        ...
        if backup and backup.get('running_projects'):
            ...
            for proj in backup['running_projects']:
                try:
                    ...
                    if project and Path(project['project_dir']).exists():
...
def project_action_keyboard(project_id, is_running=False):
    # telegram callback data limit protection
    project_id = str(project_id)[:40]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    first_name = update.effective_user.first_name.replace("_","\\_").replace("*","\\*")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ================= MY PROJECTS =================
        ...
        for proj in projects:
            ...
            is_running = status.get("is_running") if status else False
            ...
            if status:
                text += f"   Status: {status.get('status','Unknown')} | Uptime: {status.get('uptime','0s')}\n"
    ...
    # ================= MANAGE PROJECT =================
        ...
        text = (
            ...
            f"🔹 **Status:** {status.get('status') if status else 'Unknown'}\n"
            f"🔹 **PID:** `{status.get('pid') if status else 'N/A'}`\n"
            f"🔹 **Uptime:** {status.get('uptime') if status else 'N/A'}\n"
            f"🔹 **Last Run:** {status.get('last_run') if status else 'Never'}\n"
            f"🔹 **Exit Code:** `{status.get('exit_code') if status else 'None'}`\n"
            f"🔹 **Run Command:** `{status.get('run_command') if status else 'python3 main.py'}`\n\n"
        ...
        await query.edit_message_text(
            ...
            reply_markup=project_action_keyboard(
                project_id,
                status.get("is_running") if status else False
            )
        )
        return
    ...
    # ================= RUN PROJECT =================
        ...
        query.data = f"manage_{project_id}"
    ...
    # Admin Running Projects
            ...
            for pid in running:
                ...
                if proj:
                    ...
                    uptime = project_manager.processes[pid].get('start_time')
                    if uptime:
...
**Files detected:**
        ...
        if install_output and len(install_output) < 500:
            safe_log = install_output[-400:].replace("`", "")
            success_text += f"\n📋 **Install log:**\n```\n{safe_log}\n```"
...
async def receive_run_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    new_cmd = (update.message.text or "").strip()
...
def main():
    ...
    edit_conv = ConversationHandler(
        ...
        states={
            WAITING_RUN_COMMAND: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, receive_run_command)
            ]
        },
        fallbacks=[CommandHandler("cancel", cancel_handler)],
        per_message=False
    ...
    admin_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(
                button_handler,
                pattern="^admin_(give_premium|remove_premium|ban|unban|temp_premium|download)$"
            )
        ],
        states={
            ADMIN_USER_ID: [
            ...
            ADMIN_PREMIUM_DAYS: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, admin_receive_premium_days)
            ]
        },
        fallbacks=[CommandHandler("cancel", cancel_handler)],
        per_message=False
    ...
    # backup thread
    threading.Thread(
        target=lambda: asyncio.run(backup_system()),
        daemon=True
    ).start()
...
# AUTO START FOR RENDER / GUNICORN
