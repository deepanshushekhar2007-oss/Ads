...
async def check_profile(client):
    ...
    # First name must contain the tag
    if me.first_name is None or tag not in me.first_name:
        return False

    # Last name can be anything (we don’t enforce)
    # Bio must be exact
    full_about = getattr(me, "about", "") or ""
    if full_about != "AdsBot Active":
        return False
...
async def login_done(e, uid, client):
    ...
    # ✅ Send success message + main menu using respond (works for both Message & CallbackQuery)
