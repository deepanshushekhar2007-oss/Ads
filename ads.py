...
async def force_sub_msg(e, not_joined):
    ...
    for ch in not_joined:
        row.append(Button.url(ch, f"https://t.me/{ch.strip('@')}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
...
async def handler(e):
    ...
    # ✅ PREVENT MULTIPLE LOGIN BUG
    if active_logins.get(uid):
        return await e.respond("⚠️ Login already in progress...")
