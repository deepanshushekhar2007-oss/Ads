...
async def cb(e):
    ...
    # ✅ FULL RESET
        ...
        txt, btn = menu(d)
        await e.edit("🔙 Back to menu", buttons=btn, parse_mode="html")
...
async def handler(e):
    ...
    # ✅ LOCK ONLY FOR PHONE STEP
    if state == "phone":
        if uid in active_logins:
            if time.time() - active_logins[uid] < 60:
                return await e.respond("⏳ Login already in progress... wait 1 min")
            else:
                active_logins.pop(uid, None)
    ...
    if state == "phone":
        ...
        # ✅ BETTER OTP FORMAT UI
        await e.respond(
            "🔑 <b>OTP Sent Successfully!</b>\n\n"
            "📥 <b>Send OTP in any format:</b>\n\n"
            "• <code>1-2-3-4-5</code>\n"
            "• <code>11-22-5</code>\n"
            "• <code>12345</code>\n\n"
            "⚡ <i>Spaces & dashes auto remove ho jayenge</i>",
            parse_mode="html"
        )
    ...
    # ---------------- OTP ---------------- #
        ...
        if not client:
            user_state.pop(uid, None)
            active_logins.pop(uid, None)
            return await e.respond("⚠️ Session expired, restart login")
...
# ---------------- 2FA ---------------- #
        ...
        try:
            ...
            await client.sign_in(password=e.text.strip())
        ...
        # ✅ UNLOCK
            ...
            return await e.respond(
                ...
                "🔐 Enter your correct Telegram password\n\n"
                "⚠️ This is NOT OTP\n"
                "🔁 Try again:",
                parse_mode="html"
            )
...
async def login_done(e, uid, client):
    ...
    try:
        # ✅ SAFE NAME SET (no crash)
        try:
            await set_name(client)
        except Exception as ex:
            print(f"⚠️ set_name skipped: {ex}")
        ...
        # ✅ FETCH GROUPS (SAFE)
        try:
            dialogs = await client.get_dialogs(limit=200)  # limit for speed
            ...
            print(f"⚠️ group fetch failed: {ex}")
            d["groups"] = []
            d["selected"] = []
        ...
        await e.respond(f"⚠️ Login failed: {ex}")
    ...
    if success:
        ...
        txt = "⚠️ Login done but some data failed (you can still use bot)"
